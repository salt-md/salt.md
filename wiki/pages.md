...
Tags are lightweight labels, Obsidian-style, that cut across the tree. They sit
under the title as coloured chips.
...
lists its rows, Notion-style, so you can walk DB → row → dossier page without
opening the table first. Two things about that list: it loads the **first 50
