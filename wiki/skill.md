...
A file named `salt-md-skill.zip`, generated at the moment you press the button.
It contains four Markdown documents in a folder called `salt-md/` — nothing
...
The browser saves `salt-md-skill.zip`. Unzip it and you get a `salt-md/` folder.
...
Opens with a small YAML header (`name: salt-md` and a description telling an
...
**Claude Code** — put the `salt-md/` folder in either place:

```
~/.claude/skills/salt-md/       every project
.claude/skills/salt-md/         this repository only
```

Then invoke it with `/salt-md`, or just ask to work with Salt.
...
playbook itself. Unpack the new bundle over the old `salt-md/` folder: it is the
