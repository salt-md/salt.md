...
Open pages are chips across the top of the page column, Obsidian-style. The
strip appears from the second tab — one document needs no tab bar.
...
A middle column, Bear-style: documents in the workspace as cards with a snippet,
a thumbnail, when the page changed and its first two tags, **most recently
changed first**. It is not quite every document — templates are left out, and so
